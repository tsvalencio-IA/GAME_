// =============================================================================
// AR TOY TRUCK SIMULATOR: MASTER ARCHITECT EDITION (V29 - UNIFIED ECONOMY)
// OFICINA MINORITY REPORT GIGANTE + INTEGRAÇÃO COM XP/MOEDAS DOS OUTROS JOGOS
// =============================================================================
...
(function() {
    ...
    const GestureOffice = {
        ...
        menus: {
            MAIN: [
                ...
                { id: 'EXIT', icon: '🚀', label: 'VOLTAR AO CHÃO', color: '#00ff66', isWide: true }
            ],
            ENGINE: [
            ...
            ADMIN: [
                ...
                { id: 'ACT_SCOUT',  action: 'scout', icon: '🕵️', label: 'OLHEIRO', desc: '+ Chance de Raros', cost: 2500, color: '#9b59b6' },
                { id: 'BACK', icon: '🔙', label: 'VOLTAR', color: '#aaaaaa', isWide: true }
            ]
        },
        ...
        update: function(ctx, w, h, dt, gameState, rawPose, drawX, drawY, scaleCanvas) {
            ...
            ctx.font = "bold clamp(30px, 8vw, 55px) 'Russo One'"; ctx.fillText(`OFICINA DO ${window.Profile?.name || 'PILOTO'}`, cx, Math.max(45, startY - 100));
            ctx.fillStyle = "#00ff66"; ctx.font = "bold clamp(24px, 6vw, 36px) 'Chakra Petch'"; ctx.fillText(`DINHEIRO: R$ ${Math.floor(currentGlobalCoins).toLocaleString()}`, cx, Math.max(80, startY - 55));
            ctx.fillStyle = "#fff"; ctx.font = "bold clamp(16px, 4vw, 24px) Arial"; ctx.fillText(`VIDA: ${Math.floor(gameState.health)}% | BATERIA: ${Math.floor(gameState.displayFuel)}/${gameState.stats.maxFuel}`, cx, Math.max(110, startY - 20));
    ...
    const Game = {
        ...
        health: 100, fuel: 100, wear: { motor: 0, wheels: 0 }, cargo: [], xp: 0,
        ...
        init: function() {
            ...
            // INTEGRAÇÃO COM O SISTEMA DE CADASTRO GLOBAL DA PLATAFORMA
            if (window.Profile && window.Profile.arSave && window.Profile.arSave.upgrades) {
                this.upgrades = window.Profile.arSave.upgrades;
            } else { this.upgrades = { engine: 1, battery: 1, radar: 1, chassis: 1, scout: false }; }
            this.applyStats();
            ...
            if (window.System && window.System.switchCamera) { window.System.switchCamera('environment').then(() => { this.loadAIModel(); }); } else { this.loadAIModel(); }
        },
        ...
        completeMission: function() {
            ...
            if (window.Profile) { window.Profile.coins = (window.Profile.coins || 0) + bonus; if(window.Profile.save) window.Profile.save(); } else { this.fallbackMoney += bonus; }
        ...
        processAR: function(ctx, w, h, dt, drawX, drawY, scaleCanvas) {
            ...
            this.detectedItems.forEach(item => {
                ...
                const cX = drawX + (item.bbox[0] * scaleCanvas) + bW/2; const cY = drawY + (item.bbox[1] * scaleCanvas) + bH/2;
