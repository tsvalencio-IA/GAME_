// =============================================================================
// AERO STRIKE AR: TACTICAL ENGINE V10 (ACE COMBAT / PS3 QUALITY)
// ARQUITETO: SENIOR GAME ENGINE ARCHITECT (DIVISÃO DE SIMULAÇÃO MILITAR)
// STATUS: TRUE 3D MATH, BANK-TO-TURN PHYSICS, FIXED YOKE COLUMN, HD HUD
// =============================================================================
...
(function() {
    ...
    // -----------------------------------------------------------------
    // 1. MOTOR 3D VETORIAL MILITAR (PRECISÃO ABSOLUTA)
    // -----------------------------------------------------------------
    const Math3D = {
        fov: 800,
        // Projeção Matemática Clássica de Câmaras 3D (Translate -> Yaw -> Pitch -> Roll)
        project: (objX, objY, objZ, camX, camY, camZ, pitch, yaw, roll, w, h) => {
            // Vetor relativo à câmara
            let dx = objX - camX;
            let dy = objY - camY; // Y cresce para cima no mundo 3D
            let dz = objZ - camZ;

            // Rotação Inversa da Câmara (para o mundo girar corretamente)
            ...
            // Cortar objetos nas costas do piloto
            if (z2 < 10) return { visible: false };

            let scale = Math3D.fov / z2;
            return {
                x: (w / 2) + (x3 * scale),
                // No Canvas, o Y cresce para baixo, logo invertemos o Y projetado
                y: (h / 2) - (y3 * scale), 
    ...
    const AudioEngine = {
        ...
        explode: function(isTank) {
            if (!this.initialized) return;
            let t = this.ctx.currentTime; let osc = this.ctx.createOscillator(); let g = this.ctx.createGain();
            osc.type = 'square'; osc.frequency.setValueAtTime(isTank? 40:80, t); osc.frequency.exponentialRampToValueAtTime(10, t + 1.0);
            g.gain.setValueAtTime(isTank? 1.0 : 0.6, t); g.gain.exponentialRampToValueAtTime(0.01, t + 1.0);
    ...
    // -----------------------------------------------------------------
    // 3. CORE DO SIMULADOR E ESTADOS
    // -----------------------------------------------------------------
    const Game = {
        state: 'START', lastTime: 0, gameMode: 'SINGLE',
        
        mission: { targetsDestroyed: 0, moneyEarned: 0, targetGoal: 30 },
        // Altitude Inicial de 1500m (Ideal para ver os tanques no chão)
        ship: { hp: 100, speed: 2000, worldX: 0, worldY: 1500, worldZ: 0, pitch: 0, yaw: 0, roll: 0 },
        
        // SISTEMA LERP & CALIBRAÇÃO CORPORAL
        baseShoulderDist: 0,
        calibTimer: 3.0,
        input: { 
            active: false,
            roll: 0,      // Inclinação do manche lida pelas mãos
            pitchVel: 0,  // Velocidade de subida/descida lida pelos ombros
            headTilt: false 
        },
        ...
        init: function(faseData) {
            ...
            this.ship = { hp: 100, speed: 2000, worldX: 0, worldY: 1500, worldZ: 0, pitch: 0, yaw: 0, roll: 0 };
            
            this.input = { active: false, roll: 0, pitchVel: 0, headTilt: false };
            ...
            for (let i = 0; i < 60; i++) this.clouds.push({ x: (Math.random() - 0.5) * 80000, y: 5000 + Math.random() * 15000, z: (Math.random() - 0.5) * 80000, size: 2000 + Math.random() * 5000 });
        ...
        initMultiplayer: function() {
            ...
            this.sessionRef.child('host').once('value').then(snap => {
                ...
                this.playersRef.child(this.myUid).set({ name: this.myName, ready: false, hp: 100, x: 0, y: 1500, z: 0, pitch: 0, yaw: 0, roll: 0 });
            });
        ...
        // --- TRACKING CORPORAL (COLUNA FIXA + PASSO) ---
        processTracking: function(pose, w, h, dt) {
            ...
            if (pose && pose.keypoints) {
                ...
                if (rw && lw && rw.score > 0.3 && lw.score > 0.3) {
                    ...
                    // ROTAÇÃO (Roll): Ângulo entre a mão esquerda e a direita. 
                    // Se a direita desce, vira à direita.
                    targetRoll = Math.atan2(ry - ly, rx - lx);
                    targetRoll = Math.max(-Math.PI/2.5, Math.min(Math.PI/2.5, targetRoll)); 

                    // PROFUNDIDADE (Pitch): Passo à frente (ombros maiores) = Mergulha (Desce)
            ...
            if (hasInput) {
                ...
                this.input.roll += (targetRoll - this.input.roll) * 8 * dt;
                this.input.pitchVel += (targetPitchVel - this.input.pitchVel) * 5 * dt;

                if (this.state === 'PLAYING') {
                    // FÍSICA BANK-TO-TURN (REALISMO)
                    // Num caça, tu inclinas o avião (Roll) e ele curva gradualmente para esse lado (Yaw)
                    this.ship.roll = this.input.roll; 
                    
                    // A curva (Yaw) é acionada pela inclinação (Roll)
                    this.ship.yaw += Math.sin(this.ship.roll) * 2.0 * dt; 

                    // O Pitch (Subir/Descer)
                    this.ship.pitch += this.input.pitchVel * dt;
                    
                    // Limitar ângulo de voo de mergulho para manter a terra visível
                ...
                if (this.state === 'PLAYING') this.ship.roll *= 0.95; // Avião nivela as asas sozinho
            }
        ...
        processCombat: function(dt, w, h) {
            let cp = Math.cos(this.ship.pitch); let sp = Math.sin(this.ship.pitch);
            let cy = Math.cos(this.ship.yaw); let sy = Math.sin(this.ship.yaw);
            
            // Vetor Frontal da Nave
            let forwardX = sy * cp; let forwardY = sp; let forwardZ = cy * cp; 
            ...
            const checkTarget = (obj, isPlayer, uid) => {
                ...
                if (p.visible && p.z > 500 && p.z < 60000) {
                    // CAIXA DE MIRA ENORME (40% do Ecrã) -> Fácil de jogar no telemóvel
            ...
            // AUTO-LOCK RÁPIDO (0.5 Segundos na Mira)
            if (this.combat.currentTarget) {
                this.combat.lockTimer += dt;
                if (this.combat.lockTimer >= 0.5) { 
                    if (!this.combat.isLocked) AudioEngine.beep(800, 0.1); // Som de Lock-on
            ...
            // MÍSSEIS (Mover a Cabeça para a Direita)
        ...
        update: function(ctx, w, h, pose) {
            ...
            if (this.entities.length < 15 && Math.random() < 0.05) {
                let spawnDist = 45000 + Math.random() * 15000;
                ...
                if (r < 0.35) { 
                    ...
                    this.entities.push({ type: 'jet_flee', x: sx, y: Math.max(2000, this.ship.worldY + (Math.random()-0.5)*8000), z: sz, vx: forwardX * speedUnits * 0.9, vy: 0, vz: forwardZ * speedUnits * 0.9, hp: 150, yaw: this.ship.yaw }); 
                } else { 
            ...
            for (let e of this.entities) {
                ...
                if (dist > 120000) { e.hp = -1; continue; } 
                ...
                if (b.isEnemy) {
                    ...
                    for (let e of this.entities) {
                        if (Math.hypot(b.x - e.x, b.y - e.y, b.z - e.z) < 1200) {
                            ...
                            if (e.hp <= 0) this.destroyTarget(e, e.type === 'tank' ? 150 : 100); break;
                        }
                    }
        ...
        renderCalibration: function(ctx, w, h) {
            ...
            ctx.fillText("Passo para Trás = Sobe ⬆️  |  Passo para Frente = Desce ⬇️", w/2, h*0.5);
        ...
        // --- RENDERIZAÇÃO ESTILO PS3 (MOBILE OPTIMIZED) ---
        renderFrame: function(ctx, w, h) {
            ...
            // CRT Scanlines FX Leves para dar look militar
        ...
        renderEntities: function(ctx, w, h) {
            ...
            // Z-Sort: Mais distante primeiro
            toDraw.sort((a, b) => b.p.z - a.p.z);

            // NÃO aplicamos Roll no Contexto aqui. O math3D.project() já calculou a rotação correta!
            toDraw.forEach(d => {
                ...
                if (d.type === 'cloud') { ctx.fillStyle = 'rgba(255, 255, 255, 0.05)'; ctx.beginPath(); ctx.arc(p.x, p.y, obj.size * s, 0, Math.PI*2); ctx.fill(); }
                    ...
                    if (isRemote || obj.type.startsWith('jet')) { 
                        ...
                        // Tanques rodam consoante o yaw deles e mantêm-se alinhados ao chão
                    ...
                    // CAIXA DE MIRA TÁTICA (Sempre visível mesmo se o inimigo for 1 pixel)
                    let isLocked = false;
                    if (this.combat.currentTarget) {
                    ...
                    if (isLocked) {
                        ...
                        ctx.strokeStyle = obj.type === 'tank' ? 'rgba(255, 100, 0, 0.8)' : 'rgba(255, 0, 0, 0.6)'; 
                        ctx.lineWidth = 2; ctx.strokeRect(p.x - bs, p.y - bs, bs*2, bs*2);
                        if (bs === 30) { ctx.fillStyle = ctx.strokeStyle; ctx.font="12px Arial"; ctx.textAlign="center"; ctx.fillText(obj.type==='tank'?"[ TANK ]":"[ JET ]", p.x, p.y+bs+15); }
                    }
                }
        ...
        renderCockpit: function(ctx, w, h) {
            // =========================================================
            // HUD ACE COMBAT (MIRA BORESIGHT E PITCH LADDER)
            // =========================================================
            ctx.save();
            ...
            // PITCH LADDER: Linhas laterais que sobem e descem com a inclinação
            ...
            // COMPASS TAPE (Navegação Superior)
            ...
            const drawBlip = (objX, objZ, color, isSquare) => {
                ...
                let localX = dx * cosY - dz * sinY; let localZ = dx * sinY + dz * cosY; // Rotação do radar
            ...
            // ==============================================================
            // O MANCHE FÍSICO CHUMBADO (A Coluna não mexe, o Volante Gira)
            // ==============================================================
            if (this.input.active) {
                ctx.save();
                
                // 1. A COLUNA CENTRAL (Fixa no meio da tela, em baixo)
                ctx.translate(w/2, h);
                
                // Profundidade (Pitch) estica ou encolhe o manche (Empurrar = Menor)
                let depthScale = 1.0;
                ...
                // 2. O VOLANTE DE VOO
                ctx.translate(0, -160); // Sobe ao topo da coluna
                ctx.rotate(this.input.roll); // Gira SOMENTE O VOLANTE
            ...
            ctx.fillStyle = '#f1c40f'; ctx.font = "bold 16px 'Russo One'"; ctx.textAlign="right"; ctx.fillText(`DINHEIRO: R$ ${this.mission.moneyEarned}`, w - 20, h - 20);
        }
    };
