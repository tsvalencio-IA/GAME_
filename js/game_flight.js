...
(function() {
    ...
    const Game = {
        ...
        init: function(faseData) {
            ...
            // Carrega Upgrades do Perfil (Persistência)
            if(window.Profile && window.Profile.flightUpgrades) {
                this.upgrades = window.Profile.flightUpgrades;
            } else {
                this.upgrades = { engine: 1, radar: 1, missile: 1, thermal: 1 };
            }
            
            // Sincronizar Horário Real do Dispositivo
            ...
            else { this.environment = { skyTop:'#000428', skyBot:'#004e92', ground:'#050505', isNight:true }; 
                   for(let i=0;i<150;i++) this.environment.stars.push({x:Math.random()*2-1, y:Math.random(), z:Math.random()*2-1}); }

            // Spawns iniciais de cenário
            for(let i=0; i<50; i++) this.spawn('cloud', { x:(Math.random()-0.5)*200000, y:8000+Math.random()*20000, z:(Math.random()-0.5)*200000, size:5000+Math.random()*10000 });
            for(let i=0; i<20; i++) this.spawn('enemy_tank', { x:(Math.random()-0.5)*150000, y:0, z:(Math.random()-0.5)*150000, hp:300 });
            for(let i=0; i<5;  i++) this.spawn('enemy_jet', { x:(Math.random()-0.5)*100000, y:15000, z:(Math.random()-0.5)*100000, hp:150 });

            this.net.uid = window.System?.playerId || "p_" + Math.floor(Math.random()*9999);
            this.mode = faseData?.mode || 'SINGLE';
            
            // Bind eventos de clique global (Mouse/Touch)
            this._handleClick = this._handleClick.bind(this);
            window.addEventListener('pointerdown', this._handleClick);

            if(this.mode !== 'SINGLE' && window.DB) this._initNet();
            else this.state = 'HANGAR'; // Single player vai direto pro Hangar
        ...
        _readPose: function(pose, w, h, dt) {
            ...
            if(rw?.score > 0.3 && lw?.score > 0.3) {
                ...
                // Deadzone e Curva de Sensibilidade para Roll
                if(Math.abs(rawRoll) < 0.1) rawRoll = 0;
                this.pilot.trgRoll = rawRoll;

                // PITCH (Arfar / Subir e Descer)
        ...
        _processPhysics: function(dt) {
            ...
            // Calcula Velocidade Máxima baseada no Upgrade de Motor
            let maxSpeed = 1800 + (this.upgrades.engine * 400);
            this.ship.speed += (maxSpeed - this.ship.speed) * dt;

            let speedMultiplier = this.ship.speed * 20;
            this.ship.x += fX * speedMultiplier * dt;
        ...
        _processCombat: function(dt, w, h, now) {
            // Upgrade do Radar afeta distância do lock-on
            ...
            for(let id in this.entities) {
                ...
                if(e.type === 'net_player' && e.id === this.net.uid) continue; // Não focar em si mesmo
                if(e.type === 'net_player' && this.mode === 'COOP') continue; // Fogo amigo desativado no COOP
                ...
                if(p.visible && p.z < radarRange) {
                    ...
                    if(dToCenter < w * 0.4) { // 40% da tela é caixa de travamento
            ...
            // AUTO-VULCAN (Disparo Automático se Travado) - Depende de Resfriamento
            ...
            if(this.combat.locked && !this.combat.isJammed && now - this.combat.vulcanCd > 100) {
                ...
                if(target) {
                    ...
                    this.spawn('bullet', { 
                        ...
                        vx: (dx/dist)*bSpeed, vy: (dy/dist)*bSpeed, vz: (dz/dist)*bSpeed,
            ...
            if(this.combat.locked && this.pilot.headTilt && this.combat.missileCd <= 0) {
                ...
                if(window.Gfx) window.Gfx.shakeScreen(4);
            }
        },

        _processAI: function(dt) {
            // Spawns dinâmicos para sempre ter inimigos
            let enemyCount = 0;
        ...
        _updateEntities: function(dt) {
            for(let id in this.entities) {
                ...
                if(e.type === 'cloud') {
                    ...
                    // FX de rastro da bala
                    if(Math.random() < 0.5) this.spawn('fx', {x:e.x, y:e.y, z:e.z, life:0.2, color: e.isEnemy ? '#ff3300' : '#ffff00', size: 100});
                    ...
                    // Dano nos Inimigos
                        ...
                        for(let tId in this.entities) {
                            ...
                            if(t.type.startsWith('enemy') && Math.hypot(e.x - t.x, e.y - t.y, e.z - t.z) < 2500) {
                                t.hp -= 20;
                                this.spawn('fx', {x:t.x, y:t.y, z:t.z, life:0.5, color:'#ff9900', size:300});
                    ...
                    e.speed += 25000 * dt; // Aceleração do míssil
                    let target = this.entities[e.targetId];
                    if(!target) { e.life = 0; delete this.entities[id]; continue; }
                    
                    let dx = target.x - e.x, dy = target.y - e.y, dz = target.z - e.z;
                    let dist = Math.hypot(dx,dy,dz);
                    let turnRate = (30000 + this.upgrades.missile * 15000) * dt;
                    
                    e.vx += (dx/dist) * turnRate; e.vy += (dy/dist) * turnRate; e.vz += (dz/dist) * turnRate;
        ...
        _explode: function(tId) {
            ...
            this.spawn('fx', {x:t.x, y:t.y, z:t.z, life:2.0, color:'#ff3300', size: 1000});
            this.spawn('fx', {x:t.x, y:t.y, z:t.z, life:1.5, color:'#ffff00', size: 600});
        ...
        _handleClick: function(e) {
            ...
            if(this.state === 'LOBBY') {
                if(x > w/2 - 150 && x < w/2 + 150 && y > h*0.8 && y < h*0.8 + 60) {
                    if(this.net.isHost) {
                        this.net.sessionRef.child('state').set('HANGAR');
                    } else {
            ...
            // Hangar Clicks via touch/mouse fallback (também suporta Mão)
        ...
        _draw: function(ctx, w, h, now) {
            ...
            grad.addColorStop(0, this.environment.skyTop);
            grad.addColorStop(1, this.environment.skyBot);
        ...
        _drawEntities: function(ctx, w, h) {
            ...
            drawList.forEach(item => {
                ...
                // Nomes e Vidas para Multiplayer
                ...
                if(this.combat.targetId === e.id) {
                    ctx.strokeStyle = this.combat.locked ? "#f03" : "#0f0";
                    ctx.lineWidth = 3;
                    ...
                    if(this.combat.locked) {
                        ctx.fillStyle = "#f03"; ctx.font = "bold 14px Arial"; ctx.textAlign = "center";
                        ctx.fillText("LOCK", p.x, p.y + sz + 20);
                    }
                }
            });
        },
        ...
        _drawMesh: function(ctx, mesh, p, scale) {
            mesh.f.forEach(f => {
                ...
                f.slice(0, -1).forEach((vIdx, i) => {
                    let v = mesh.v[vIdx];
                    let sx = p.x + v.x * p.s * (scale);
                    let sy = p.y - v.y * p.s * (scale);
        ...
        _drawHUD: function(ctx, w, h, now) {
            ...
            for(let i=-90; i<=90; i+=10) {
                ...
                ctx.stroke();
                ctx.setLineDash([]);
                ctx.fillStyle = "rgba(0, 255, 100, 0.8)"; ctx.font = "bold 12px 'Chakra Petch'";
            ...
            ctx.textAlign = "right";
            ctx.fillStyle = this.ship.hp > 30 ? "#0f6" : "#f00";
            ctx.fillText(`HP: ${Math.floor(this.ship.hp)}%`, w - 20, 40);
            ctx.fillStyle = "#f1c40f";
            ctx.fillText(`R$ ${window.Profile?.coins || 0}`, w - 20, 65);
            ctx.fillStyle = "#fff"; ctx.font = "bold 14px Arial";
            ctx.fillText(`ABATES: ${this.session.kills} / ${this.session.goal}`, w - 20, 85);

            // BARRAS DE STATUS INFERIORES
            ...
            if(!this.pilot.active) {
                ctx.fillStyle = "rgba(255, 0, 0, 0.6)"; ctx.fillRect(0, cy-30, w, 60);
                ctx.fillStyle = "#fff"; ctx.font = "bold 25px Arial";
                ctx.fillText("MÃOS FORA DA CÂMERA", cx, cy+10);
            }
        },
        ...
        _drawLobby: function(ctx, w, h) {
            ...
            for(let id in this.net.players) {
                let p = this.net.players[id];
                ctx.fillStyle = p.ready ? "#2ecc71" : "#e74c3c";
                ctx.fillText(`${p.name} - ${p.ready ? 'PRONTO' : 'ESPERANDO'}`, w/2, py); py += 35;
            }

            let bColor = this.net.isReady ? "#e67e22" : "#27ae60";
            if(this.net.isHost) bColor = "#c0392b";
        ...
        _getHangarItems: function(w, h) {
            return [
                ...
                { id: 'start', name: this.net.isHost ? '>> INICIAR VOO <<' : '>> AGUARDANDO HOST <<', cost: 0, lvl: 0, max: 0, y: h*0.85, isBtn: true }
            ];
        },
        ...
        _drawHangar: function(ctx, w, h, dt) {
            ...
            if (this.pilot.active) { 
                ctx.fillStyle = '#e74c3c'; ctx.beginPath(); ctx.arc(this.pilot.wristR.x, this.pilot.wristR.y, 15, 0, 7); ctx.fill(); 
            }
        },
        ...
        _buyItem: function(item) {
            if(item.isBtn) {
                if(this.mode === 'SINGLE' || this.net.isHost) {
                    if(this.mode !== 'SINGLE') this.net.sessionRef.child('state').set('CALIBRATION');
                    else { this.state = 'CALIBRATION'; this.timer = 4; }
                    if(window.Sfx) window.Sfx.play(1000, 'sine', 0.2);
                }
            } else if (window.Profile && window.Profile.coins >= item.cost && this.upgrades[item.id] < item.max) {
                ...
                if(window.Sfx) window.Sfx.play(300, 'square', 0.2);
            }
        },
        ...
        _drawCalib: function(ctx, w, h) {
            ...
            ctx.fillText("POSICIONE AS MÃOS À FRENTE", w/2, h*0.5);
            ctx.strokeStyle = "#0f6"; ctx.strokeRect(w*0.2, h*0.7, w*0.6, 20);
            ctx.fillStyle = "#0f6"; ctx.fillRect(w*0.2, h*0.7, (w*0.6) * (1 - this.timer/4), 20);
        },
        ...
        _drawEnd: function(ctx, w, h) {
            ...
            ctx.fillStyle = "#fff"; ctx.font = "bold 20px Arial";
            ctx.fillText(`INIMIGOS DESTRUÍDOS: ${this.session.kills}`, w/2, h/2 + 50);
        },
